...
import { getTablesForCaterer, markTableServed, unmarkTableServed } from "@/lib/actions/table-actions";
import { toast } from "sonner";
import { ChefHat, Check, Utensils, Users, Clock, Undo2, ChevronDown, ChevronUp, Eye, EyeOff } from "lucide-react";
...
export function CatererPanel() {
  ...
  async function loadTables() {
    ...
    setTables(data.map((t) => ({ ...t, servedAt: t.servedAt ? t.servedAt.toISOString() : null })));
  }
  ...
  return (
    <div className="min-h-screen px-4 pt-6 pb-12 max-w-2xl mx-auto">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-gold-pale/50 text-xs mt-1">Auto-refreshes every 5 seconds</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { icon: Utensils, value: tables.length,        label: "Total Orders", color: "text-gold-light" },
          { icon: Clock,    value: pendingTables.length, label: "Pending",      color: "text-amber-400"  },
          { icon: Check,    value: servedTables.length,  label: "Served",       color: "text-emerald-400" },
        ].map(({ icon: Icon, value, label, color }) => (
          <div key={label} className="bg-royal-purple/30 border border-gold/15 p-3 text-center">
            <Icon className={`w-4 h-4 mx-auto mb-1 ${color}`} />
            <div className={`font-[family-name:var(--font-heading)] text-2xl ${color}`}>{value}</div>
            <div className="text-[0.6rem] tracking-[0.2em] uppercase text-gold-pale/50">{label}</div>
          </div>
        ))}
      </div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {pendingTables.map((table) => {
              ...
              return (
                <div key={table.id} className="bg-royal-purple/30 border border-gold/15 relative overflow-hidden">
                  <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-amber-400 to-transparent" />

                  <div className="p-4 sm:p-5">
                    {/* Table header — stacks on mobile, side-by-side on sm+ */}
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                      ...
                      <Button
                        onClick={() => handleMarkServed(table.id, table.number)}
                        disabled={isPending}
                        className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 text-white font-[family-name:var(--font-display)] text-[0.7rem] tracking-[0.15em] uppercase min-h-[48px] sm:min-h-0"
                      >
                    ...
                    <div className="space-y-2 mb-3">
                      {summary.map(({ name, count }) => (
                        <div key={name} className="flex items-center justify-between py-1">
                          ...
                          <span className="text-gold font-[family-name:var(--font-heading)] text-xl">
                            ×{count}
                          </span>
                        </div>
                      ))}
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 text-gold-pale/40 hover:text-gold-pale/70 text-xs transition-colors min-h-[36px]"
                    >
                      {isExpanded ? (
                        <><ChevronUp className="w-3.5 h-3.5" /> Hide details</>
                      ) : (
                        <><ChevronDown className="w-3.5 h-3.5" /> Show person-by-person</>
                      )}
                    </button>
                      ...
                      <div className="mt-3 pt-3 border-t border-gold/10 space-y-2">
                        {table.orders.map((order) => (
                          <div key={order.id} className="flex items-center justify-between py-0.5">
                            <span className="text-gold-pale/60 text-sm">{order.personName}</span>
        ...
        <div>
          ...
          <div className="space-y-3">
            {servedTables.map((table) => {
              const summary = getMealSummary(table.orders);
              return (
                <div key={table.id} className="bg-royal-purple/15 border border-gold/8 relative overflow-hidden opacity-60">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="flex items-center gap-1 text-emerald-400/60 text-xs">
                          ...
                          {table.servedAt && (
                            <span className="ml-1">
                              {new Date(table.servedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                            </span>
                          )}
                        </span>
                      </div>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleUnmarkServed(table.id, table.number)}
                        disabled={isPending}
                        className="text-gold-pale/30 hover:text-gold-pale/60 text-xs min-h-[44px]"
                      >
      ...
      {tables.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="font-[family-name:var(--font-heading)] text-lg text-gold-pale/40">No orders yet</p>
          <p className="text-gold-pale/30 text-sm mt-1">Orders will appear here when guests submit them</p>
        </div>
      )}
