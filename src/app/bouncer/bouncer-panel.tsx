...
export function BouncerPanel() {
  ...
  const [optimisticGuests, setOptimisticGuests] = useOptimistic(
    ...
    (
      state,
      action:
        | { type: "flag"; id: string; flagged: boolean }
        | { type: "checkin"; id: string; checkedIn: boolean }
    ) => {
      if (action.type === "flag") {
        return state.map((g) =>
          g.id === action.id
            ? { ...g, flaggedByBouncer: action.flagged }
            : g
        );
      }
      return state.map((g) =>
        g.id === action.id ? { ...g, checkedIn: action.checkedIn } : g
      );
    }
  );
  ...
  function handleFlag(guestId: string, flagged: boolean) {
    startTransition(async () => {
      setOptimisticGuests({ type: "flag", id: guestId, flagged });
      await flagGuest(guestId, flagged);
      await loadData();
      toast.success(flagged ? "Guest flagged — no access card" : "Flag removed");
    });
  }
  ...
  function handleCheckIn(guestId: string, guestName: string, checkedIn: boolean) {
    startTransition(async () => {
      ...
      await loadData();
      toast.success(
        checkedIn ? `${guestName} checked in` : `${guestName} check-in undone`
      );
    });
  }

  const attendingGuests = optimisticGuests.filter(
  ...
  // Sort: unchecked first, then checked (greyed out at bottom)
    ...
    <div className="min-h-screen px-4 pt-6 pb-12 max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        <div className="bg-royal-purple/30 border border-gold/15 p-3 text-center">
          ...
          <div className="text-[0.55rem] tracking-[0.15em] uppercase text-gold-pale/50">
            Expected
          </div>
        </div>
        <div className="bg-royal-purple/30 border border-gold/15 p-3 text-center">
        ...
        <div className="bg-royal-purple/30 border border-gold/15 p-3 text-center">
          ...
          <div className="font-[family-name:var(--font-heading)] text-2xl text-red-400">
            {stats.flagged}
          </div>
          <div className="text-[0.55rem] tracking-[0.15em] uppercase text-gold-pale/50">
            No Card
          ...
          sortedGuests.map((guest) => (
            ...
            >
              <div
                ...
                onClick={() =>
                  handleCheckIn(guest.id, guest.name, !guest.checkedIn)
                }
              >
                <p
                  className={`font-[family-name:var(--font-heading)] text-lg truncate transition-all ${
                    guest.checkedIn
                      ? "text-gold-pale/40 line-through"
                      : "text-cream"
                  }`}
                >
                  {guest.name}
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xs text-gold-pale/50">
                    {guest.guestCount}{" "}
                    {guest.guestCount === 1 ? "person" : "people"}
                  </p>
                  {guest.checkedIn && (
              ...
              <div className="flex items-center gap-2 ml-3">
                {!guest.checkedIn && (
                  <Button
                    ...
                    disabled={isPending}
                    onClick={() =>
                      handleFlag(guest.id, !guest.flaggedByBouncer)
                    }
                  ...
                  >
                    ...
                    {guest.flaggedByBouncer ? "Flagged" : "No Card"}
                  </Button>
                )}
